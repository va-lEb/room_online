...
// Enregistrement et modification d'un produit
...
$liste_produits = $pdo->query("SELECT id_produit, produit.id_salle, date_format(date_arrivee, '%d/%m/%Y') AS date_arrivee_fr, date_format(date_depart, '%d/%m/%Y') AS date_depart_fr, prix, etat, photo, titre, categorie FROM produit INNER JOIN salle ON produit.id_salle = salle.id_salle ORDER BY id_produit");
