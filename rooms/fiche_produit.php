...
$liste_avis = $pdo->query(" SELECT avis.id_membre, avis.id_salle, avis.commentaire, membre.pseudo, avis.date_enregistrement, avis.note FROM avis INNER JOIN membre ON avis.id_membre = membre.id_membre WHERE avis.id_salle = $infos_produit[id_salle]");
                ...
                <div class="p-2 d-flex flex-wrap align-items-center">
                    ...
                    <div class="ms-auto">
                        <?php if(user_is_connected()) {
                            // Condition pour cacher le bouton si accès depuis la page gestion_produit et produit deja réservé
                            if($infos_produit['etat'] == 'libre') {
                                echo '<a href="?action=reserver&id_produit=' . $infos_produit['id_produit'] . '" title="reserver" class="btn btn-dark confirm_resa">Réserver</a>';
                            } else {
